...
const mockQueue: QueueItem[] = [
  { id: "1", influencer: "Sarah Chen", handle: "@sarah.creates", avatar: "SC", business: "The Rooftop Cafe", gig: "Summer Brunch Promo", path_type: "content", content_url: "https://instagram.com/reel/abc123", screenshot: true, feedback: null, review_status: "submitted", submitted_at: "2h ago", reward_type: "paid", budget: 100, base_percent: 30, platform_fee: 5, resubmission_count: 0 },
  { id: "2", influencer: "Alex Rivera", handle: "@alexeats", avatar: "AR", business: "The Rooftop Cafe", gig: "Summer Brunch Promo", path_type: "feedback", content_url: null, screenshot: false, feedback: { rating: 2, worked: "Great atmosphere and decor", didnt: "Wait time was 40+ minutes, food was cold", suggestions: "Assign a dedicated staff for influencer visits" }, review_status: "submitted", submitted_at: "5h ago", reward_type: "paid", budget: 100, base_percent: 30, platform_fee: 5, resubmission_count: 0 },
  { id: "3", influencer: "Maya Johnson", handle: "@maya.jfood", avatar: "MJ", business: "Sakura Sushi", gig: "New Menu Tasting", path_type: "content", content_url: "https://tiktok.com/@maya/video/456", screenshot: true, feedback: null, review_status: "submitted", submitted_at: "1d ago", reward_type: "free", budget: 0, base_percent: 0, platform_fee: 0, resubmission_count: 1 },
];
...
export default function AdminReviewPage() {
  ...
  function payoutSummary(item: QueueItem): string {
    ...
    if (item.path_type === "content") {
      return `Release $${(base + completion - item.platform_fee).toFixed(2)} to influencer ($${item.platform_fee} platform fee)`;
    }
    return `Release $${(base - item.platform_fee).toFixed(2)} base to influencer; refund $${completion.toFixed(2)} to business`;
  }
    ...
    <div className="flex min-h-screen items-start justify-center px-6 pt-6 lg:pt-12">
      <div className="w-full max-w-[640px] pb-16">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-xl font-extrabold tracking-tight">Review Queue</h1>
                ...
                <CardContent className="p-0">
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">{item.business} · {item.gig} · {item.submitted_at}</div>
                    </div>
                    ...
                    <div className="animate-fade-up border-t border-border px-4 pb-4 pt-3">
                      ...
                      {item.path_type === "content" && item.content_url && (
                        <div className="space-y-3">
                          <div>
                            <div className="font-display text-[11px] font-semibold uppercase text-muted-foreground">Content Link</div>
                          ...
                          {item.screenshot && (
                            <div className="flex h-32 items-center justify-center rounded-lg bg-muted">
                              ...
                              <span className="ml-2 text-xs text-muted-foreground">Screenshot attached</span>
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      {item.path_type === "feedback" && item.feedback && (
                        <div className="space-y-3">
                          <div className="flex items-center gap-2">
                            <span className="font-display text-xs font-semibold text-muted-foreground">Rating:</span>
                            <div className="flex gap-0.5">
                              {[1,2,3,4,5].map((n) => (
                                <div key={n} className={`h-5 w-5 rounded text-center text-[11px] font-bold leading-5 ${n <= item.feedback!.rating ? "bg-amplii-amber text-white" : "bg-muted text-muted-foreground"}`}>{n}</div>
                              ))}
                            </div>
                          </div>
                          <div>
            ...
            <DialogContent className="sm:max-w-[420px]">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-muted-foreground">{actionDialog.item.gig}</span>
                </div>
